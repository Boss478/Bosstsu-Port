...
+ **LawLib FULL/COMPACT merge — digest into the law reader (5 scrutiny loops, 38 findings, 2 senior reviews)**:
  ...
  + * **Pre-existing blocker fixed**: turbopack NFT build failure (`upload.ts` dynamic path traced 13,886 files) — 8 `/*turbopackIgnore: true*/` comments (Next's documented fix); build restored.
  + * **Tests/evals**: +6 vitest (digest tokens/flags/hasCard/dline ids, per-slug view key whitelist 4) → 175/175; eval 17/17 incl. new built-HTML hybrid-region graders; full QA checklist verified in production build (hover/Escape/suppression/TOC jump/search jump/deep links/flash via MutationObserver).

    ...
    },
    {
      ...
      check: (root) => {
        ...
        try {
          ...
          if (servers && typeof servers === 'object' && Object.keys(servers).length >= 1) {
            return { pass: true, path: configPath, detail: `MCP servers configured (${Object.keys(servers).length})` };
          }
        } catch {}
        try {
          ...
          const gServers = globalParsed?.mcp?.servers;
          if (gServers && typeof gServers === 'object' && Object.keys(gServers).length >= 1) {
            return { pass: true, path: globalPath, detail: `MCP servers via global (${Object.keys(gServers).length})` };
          }
        } catch {}
        return { pass: false, path: configPath, detail: 'No MCP section found' };
      },
    },
    {
    ...
    },
    {
      ...
      check: (root) => {
        ...
        try {
          ...
          const limited = Array.isArray(perms) && perms.some((p) => p && p.action === 'external_directory');
          return { pass: limited, path: configPath, detail: limited ? 'External directory scoped' : 'No external_directory restriction' };
        } catch {
          return { pass: false, path: configPath, detail: 'No external_directory restriction' };
        }
      },
    },

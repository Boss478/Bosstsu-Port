/**
 * KruLAW — PDF text extraction helper (assist-only, NOT a content source).
 *
 * Run: npx tsx scripts/krulaw/extract-pdf.ts <input.pdf> <output.md>
 *
...
function usage(): never {
  console.error('Usage: npx tsx scripts/krulaw/extract-pdf.ts <input.pdf> <output.md>');
  process.exit(1);
}
...
> Tool: scripts/krulaw/extract-pdf.ts (pdfjs-dist legacy build, Node, no OCR)

/**
 * KruLAW — build pipeline CLI.
 *
 * Usage: npx tsx scripts/krulaw/build.ts [--check] [--include-sample]
 *
 *  - Reads every `*.md` in content/krulaw/laws/ EXCEPT `_`-prefixed files.
 ...
 *  - Emits: src/data/krulaw/laws/<slug>.json (LawDoc),
 *    src/data/krulaw/index.json (list metadata),
 *    src/data/krulaw/registry.ts (static-literal lazy loaders ONLY — never
 *    template-literal dynamic imports, webpack-safe).
 *
 * Thin CLI — all real logic lives in src/lib/krulaw/*.
 */
...
function main(): void {
  ...
  for (const { file, doc } of passing) {
    ...
    console.log(
      `[BUILT] ${file} → src/data/krulaw/laws/${doc.slug}.json (${articleCountOf(doc)} มาตรา)`,
    );
  }
  ...
  const registry = `// AUTO-GENERATED by \`npm run krulaw:build\` — do not edit manually.
...
`;
  ...
  console.log(`[EMIT] src/data/krulaw/index.json (${indexEntries.length} law(s))`);
  console.log('[EMIT] src/data/krulaw/registry.ts');
}

...
const learningData = [
  {
    ...
    description:
      'แผนการสอนรายสัปดาห์สำหรับวิชาคณิตศาสตร์ ชั้น ป.1 เรื่องการบวกและการลบ numbers 1-20',
    subject: 'คณิตศาสตร์',
    type: 'แผนการสอน',
...
async function seed() {
  ...
  const ToolSession =
    mongoose.models.ToolSession || mongoose.model('ToolSession', ToolSessionSchema);
  const ToolStepTemplate =
    mongoose.models.ToolStepTemplate || mongoose.model('ToolStepTemplate', ToolStepTemplateSchema);
  const WordOverrideSchema = new mongoose.Schema(
    {
      slug: { type: String, required: true, unique: true },
  ...
  const StockHolding =
    mongoose.models.StockHolding || mongoose.model('StockHolding', StockHoldingSchema);
  const WordOverride =
    mongoose.models.WordOverride || mongoose.model('WordOverride', WordOverrideSchema);
  ...
  // Seed word overrides
  console.log('Seeding word overrides...');
  const wordOverrideData = [
    {
      slug: 'cat-a1',
      word: 'cat',
      level: 'a1',
      ...
      syllables: ['cat'],
      phonemes: ['k', 'ae', 't'],
      definition: 'A small domesticated carnivorous mammal with soft fur. Updated by override!',
    ...
    },
    {
      ...
      ipaUs: '/dɔːɡ/',
      definition:
        'A domesticated carnivorous mammal that typically has a long snout. Override test.',
      example: 'The dog barked loudly.',
      published: false,
    },
  ];
  await WordOverride.insertMany(wordOverrideData);
  console.log(`  ✓ Inserted ${wordOverrideData.length} word overrides`);
  ...
  console.log(`   Word Overrides: ${wordOverrideData.length}`);

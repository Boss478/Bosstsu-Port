// ===========================================================================
// KruLAW L1 — parser TDD spec (QA-first; contract per
// .agents/plans/kruexam-krulaw.md §4 + .agents/tasks/krulaw-execution.md,
// 11-case checklist).
//
...
import { parseLawMarkdown } from '@/lib/krulaw/parser';
...
//
// NOTE: `Chapter.sections` is a GATE-1 follow-up extension (types/krulaw.ts is
// FROZEN at GATE 1). These tests are EXPECTED-RED until the extension lands —
// they pin the intended contract (see .agents/plans/krulaw-parser-tdd-spec.md
...
// See .agents/plans/krulaw-parser-tdd-spec.md §3 (updated 2026-08-04).
// ---------------------------------------------------------------------------
...
วรรคสามของมาตรานี้`),
  ...
  it('parses the real มาตรา 38 multi-marker block into TWO amendedBy entries (markers not in body)', () => {
    // Verbatim structure of content/krulaw/laws/national-education-act-2542.md
...
// content/krulaw/laws/national-education-act-2542.md verbatim shapes:
...
// ---------------------------------------------------------------------------
// DIGEST — minimal parse (L1 cut-line; .agents/tasks/krulaw-execution.md §Phase 1)
//
// Digest sources (content/krulaw/digests/*.md) get headings + [[ref]]/
...
// src/lib/krulaw/parser.ts; the pins below are un-skipped and part of the
// green gate (143/143).
// ---------------------------------------------------------------------------
...
describe('parseDigestMd — minimal digest parse', () => {
  const getFn = async (): Promise<unknown> => {
    const mod = await import('@/lib/krulaw/parser');

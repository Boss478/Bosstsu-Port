...
import { deletePortfolioItem, togglePublished } from './actions';
...
export default async function PortfolioListPage({
  ...
  const [items, total] = await Promise.all([
    Portfolio.find(query).sort(sortQuery).skip(skip).limit(limit).lean(),
                  ...
                  >
                    ...
                    <td className="p-4 hidden md:table-cell">
                      <div className="flex flex-wrap gap-1">
                        {item.tags?.slice(0, 3).map((tag: string) => (
                          <span
                            key={tag}
                            className="text-[10px] px-2 py-0.5 rounded-full bg-zinc-100 dark:bg-slate-700 text-zinc-600 dark:text-zinc-300 border border-zinc-200 dark:border-slate-600"
                          >
                            {tag}
                          </span>
                        ))}
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-2 transition-opacity">
                        ...
                        <DeleteButton id={item._id.toString()} action={deletePortfolioItem} />
                      </div>
                    </td>
                  </tr>
                ))

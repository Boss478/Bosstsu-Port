...
export default async function LearningListPage({
  ...
  const [items, total] = await Promise.all([
    Learning.find(query).sort(sortQuery).skip(skip).limit(limit).lean(),
    Learning.countDocuments(query),
  ]);
  ...
  return (
    <div className="min-h-screen bg-sky-50 dark:bg-slate-950 pt-28 pb-12 px-4">
      <div className="max-w-6xl mx-auto">
        <Breadcrumb items={[{ label: 'Backend', href: '/admin' }, { label: 'Learning' }]} />

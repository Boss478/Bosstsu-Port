        ...
        {roundData.grid.map((item, index) => {
          ...
          return (
            <button
              key={index}
              type="button"
              disabled={item.isCorrect || !item.isHidden}
              className={`min-w-[48px] min-h-[48px] aspect-square flex items-center justify-center rounded-lg md:rounded-xl text-xl md:text-2xl font-black transition-all duration-300 cursor-pointer appearance-none ${
                item.isHidden
                  ? isActive
                  ...
                  : item.isCorrect
                    ? 'bg-emerald-500 text-white scale-105'
                    : 'bg-white dark:bg-zinc-800 border border-zinc-100 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400'
              }`}
              onClick={() =>
                !isTransitioning &&
                !isFeedbackVisible &&

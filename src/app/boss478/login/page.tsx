...
export default function PrivateLoginPage() {
  const [state, formAction, isPending] = useActionState(loginPrivate, {});
      ...
      <div className="w-full max-w-sm">
        <div className="p-8 rounded-2xl border border-white/60 dark:border-slate-700/50 bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm shadow-lg">
          ...
          <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-2xl bg-blue-600 dark:bg-blue-700 flex items-center justify-center text-white mx-auto mb-4">
              <i aria-hidden="true" className="fi fi-sr-stats text-2xl" />
            </div>
          ...
          <form action={formAction} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isPending}
              className="w-full py-3 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-semibold transition-colors flex items-center justify-center gap-2 cursor-pointer"
            >
              {isPending ? (
                <>

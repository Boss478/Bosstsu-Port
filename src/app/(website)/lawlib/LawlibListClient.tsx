...
/**
 * LawLib — law list client (FR1 + SCRUTINY-L1-3 full-text promotion).
 *
 ...
 * to `/lawlib/<slug>#มาตรา-N`. Combined results capped at 20.
 *
...
export interface LawlibIndexEntry {
  slug: string;
  code: string;
  titleTh: string;
  subject: string;
...
async function loadBuiltLaws(slugs: string[]): Promise<LawDoc[]> {
  ...
  if (missing.length > 0) {
    const mod = await import('@/data/lawlib/registry');
    await Promise.all(
...
export default function LawlibListClient({ laws }: { laws: LawlibIndexEntry[] }) {
  ...
  const groups = useMemo<PartGroup[]>(() => {
    ...
    const byPart = new Map<'ก' | 'ข', Map<string, LawlibIndexEntry[]>>();
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    {subject.laws.map((law) => (
                      <Link
                        ...
                        className="group lawlib-list-card rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-[border-color,transform,box-shadow] hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md dark:border-slate-700 dark:bg-slate-900 dark:hover:border-blue-500/60"
                      >
        ...
        <div className="space-y-4">
          ...
          {shownMeta.map((law) => (
            <Link
              key={law.slug}
              href={`/lawlib/${law.slug}`}
              className="group block lawlib-list-card rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-[border-color,transform,box-shadow] hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md dark:border-slate-700 dark:bg-slate-900 dark:hover:border-blue-500/60"
            >
              <h4 className="text-[15px] font-bold leading-relaxed text-slate-900 group-hover:text-blue-800 dark:text-slate-100 dark:group-hover:text-blue-300">
                {law.titleTh}
              </h4>
          ...
          {shownFullText.map((r) => (
            <Link
              key={`ft-${r.slug}`}
              href={`/lawlib/${r.slug}#มาตรา-${r.hits[0].articleKey}`}
              className="group block lawlib-list-card rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-[border-color,transform,box-shadow] hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md dark:border-slate-700 dark:bg-slate-900 dark:hover:border-blue-500/60"
            >
              <h4 className="text-[15px] font-bold leading-relaxed text-slate-900 group-hover:text-blue-800 dark:text-slate-100 dark:group-hover:text-blue-300">
                {r.titleTh}

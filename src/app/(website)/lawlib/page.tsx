...
import LawlibListClient, { type LawlibIndexEntry } from './LawlibListClient';

export const metadata: Metadata = {
  title: 'LawLib — คลังกฎหมาย',
  description:
...
// Reads src/data/lawlib/index.json via node fs (never a static import — the
...
// index must never crash the list render — LawlibListClient reads every
...
function isLawlibIndexEntry(item: unknown): item is LawlibIndexEntry {
...
function readLawlibIndex(): LawlibIndexEntry[] | null {
  try {
    const raw = fs.readFileSync(path.join(process.cwd(), 'src/data/lawlib/index.json'), 'utf8');
    const parsed: unknown = JSON.parse(raw);
    if (!Array.isArray(parsed)) return null;
    return parsed.filter(isLawlibIndexEntry);
  } catch {
    return null;
  }
}

export default function LawlibIndexPage() {

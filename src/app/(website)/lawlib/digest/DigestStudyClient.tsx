...
/**
 * LawLib — digest study-mode renderer (L5). Receives the server-built
 ...
 * digest (navbar hidden there, no dock/gear). The `lawlib-theme-fab` class is
...
function ThemeFab() {
  ...
  return (
    <button
      ...
      className="lawlib-theme-fab fixed right-6 bottom-32 z-50 flex h-11 w-11 items-center justify-center rounded-2xl border border-slate-200 bg-white/90 text-slate-700 shadow-md backdrop-blur transition-colors hover:border-blue-300 hover:text-blue-700 md:right-10 dark:border-slate-700 dark:bg-slate-900/90 dark:text-slate-200 dark:hover:border-blue-500/60"
    >
...
function Line({ line }: { line: RenderLine }) {
  if (line.kind === 'article') {
    return (
      <div className="lawlib-digest-card mt-4 rounded-xl border border-blue-100 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
        <Link
          href={line.href}

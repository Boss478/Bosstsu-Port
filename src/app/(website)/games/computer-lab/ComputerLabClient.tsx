...
export default function ComputerLabClient() {
  ...
  const navigate = useCallback(
    (target: Screen) => {
      ...
      if (stageScreenSet.has(target)) {
        trackCustomEvent('game_stage_start', { game: 'computer-lab', stage: target });
      } else if (target === 'victory') {
        trackCustomEvent('game_complete', { game: 'computer-lab' });
      }
      setTimeout(() => {
        setScreen(target);
        setLoading(false);
    ...
    [screen, stageScreenSet, trackCustomEvent],
  );
  ...
  const handleBootComplete = useCallback(() => {
    ...
    navigate('menu');
    trackCustomEvent('game_start', { game: 'computer-lab' });
  }, [navigate, trackCustomEvent]);
  ...
  const handleStageComplete = useCallback(
    (stageId: string, stars: number, score: number) => {
      setSaveData((prev) => {
      ...
      trackCustomEvent('game_stage_complete', {
        game: 'computer-lab',
        stage: stageId,
        stars,
        score,

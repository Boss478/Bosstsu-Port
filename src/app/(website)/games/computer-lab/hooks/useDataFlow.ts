...
function computeUtilization(
  ...
  const baseCpuLoad = runningApps.reduce(
    (sum, app) => sum + (app.active ? (appCpuCost[app.id] ?? 5) : 0),
    0,
  );
  ...
  const cpuUtil = Math.max(
    2,
    Math.min(100, activePacketsCount > 0 || baseCpuLoad > 0 ? scaledCpuUtil + cpuJitter : 2),
  );
  ...
  const baseGpuLoad = runningApps.reduce(
    (sum, app) => sum + (app.active ? (appGpuCost[app.id] ?? 0) : 0),
    0,
  );
  const graphicsPackets = packets.filter(
    (p) => p.status !== 'done' && p.route === 'graphics',
  ).length;
  ...
  const gpuUtil = Math.max(
    0,
    Math.min(100, graphicsPackets > 0 || baseGpuLoad > 0 ? scaledGpuUtil + gpuJitter : 0),
  );
  ...
  const activeSsdPackets = packets.filter(
    (p) =>
      p.status !== 'done' && (p.toId === 'ssd' || p.fromId === 'ssd' || p.busId === 'bus_cpu_ssd'),
  ).length;
  const activeHddPackets = packets.filter(
    (p) =>
      p.status !== 'done' && (p.toId === 'hdd' || p.fromId === 'hdd' || p.busId === 'bus_cpu_hdd'),
  ).length;
  ...
  const ssdUtil = Math.max(
    0,
    Math.min(100, activeSsdPackets > 0 ? Math.round(ssdUtilPercent + ssdJitter) : 0),
  );
  ...
  const hddUtil = Math.max(
    0,
    Math.min(100, activeHddPackets > 0 ? Math.round(hddUtilPercent + hddJitter) : 0),
  );
...
const getHopsForRoute = (
  route: RouteType,
  fromId: string,
  toId: string,
  ssdUsage: number,
  hddUsage: number,
): { from: string; to: string; busId: string }[] => {
  if (route === 'storage') {
...
export function useDataFlow({
  ...
  const routeTypeCountersRef = useRef<Record<RouteType, number>>({
    compute: 0,
    storage: 0,
    graphics: 0,
  });
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      if (spawnBudget > 0) {
        ...
        if (activeWorkloads.length > 0) {
          for (const wl of activeWorkloads) {
            ...
            for (let i = 0; i < wl.concurrency && newPackets.length < spawnBudget; i++) {
              ...
              const hops =
                route === 'storage' && targetStorage === 'hdd'
                  ? [
                      { from: 'keyboard', to: 'cpu', busId: 'bus_keyboard_cpu' },
                      { from: 'cpu', to: 'hdd', busId: 'bus_cpu_hdd' },
                      { from: 'hdd', to: 'cpu', busId: 'bus_cpu_hdd' },
              ...
              busOccupancyRef.current[firstHop.busId] =
                (busOccupancyRef.current[firstHop.busId] ?? 0) + 1;
              newPackets.push(pkt);
            }
          }
        }
        ...
        if (activeApps.length > 0 && newPackets.length < spawnBudget) {
          const perAppBudget = Math.max(
            1,
            Math.floor((spawnBudget - newPackets.length) / activeApps.length),
          );
          for (const app of activeApps) {
            ...
            for (let i = 0; i < appSpawnCount; i++) {
              ...
              const pkt: FlowPacket = {
                ...
                busId: firstHop.busId,
                byteValue:
                  dataSizeRef.current === 'mb' ? 1048576 : dataSizeRef.current === 'kb' ? 1024 : 1,
        ...
        if (pkt.status === 'traveling') {
          ...
          if (progress >= 1) {
            ...
            if (!alreadyArrived) {
              ...
              busOccupancyRef.current[pkt.busId] = Math.max(
                0,
                (busOccupancyRef.current[pkt.busId] ?? 1) - 1,
              );
              ...
              componentQueuesRef.current[pkt.toId] = [
                ...existingQueue,
                { ...pkt, id: `q_${pkt.id}`, progress: 0, status: 'comp_queued' as const },
              ];
        ...
        for (const pkt of queue) {
          if (pkt.status === 'processing') {
            ...
            if (nextProgress >= 1) {
              ...
              if (
                (compId === 'ssd' && ssdUsageRef.current >= 100) ||
                (compId === 'hdd' && hddUsageRef.current >= 100)
              ) {
                continue;
              }

              const hops = getHopsForRoute(
                pkt.route,
                pkt.fromId,
                pkt.toId,
                ssdUsageRef.current,
              ...
              const currentHopIdx = hops.findIndex(
                (h) => h.from === pkt.fromId && h.to === pkt.toId,
              );
              ...
              if (nextHop) {
                ...
                busOccupancyRef.current[nextHop.busId] =
                  (busOccupancyRef.current[nextHop.busId] ?? 0) + 1;
                const hopPkt = {
                  ...pkt,
      ...
      if (crashCheckTimer > 2000 && !crashActiveRef.current) {
        ...
        if (bn?.type === 'thermal' && bn.severity === 'severe') {
          ...
          if (timers.thermal >= 3000) {
            setCrashEvent({
              type: 'overheat',
              title: 'Overheating',
              desc: 'CPU temperature exceeds safe limit — system will shut down',
              fix: 'Reduce CPU cores/clock or increase fan speed',
            });
        ...
        if (ramUsageRef.current >= 100) {
          ...
          if (timers.ram >= 3000) {
            setCrashEvent({
              type: 'freeze',
              title: 'System Freeze',
              desc: 'RAM is completely full — system becomes unresponsive',
              fix: 'Close running apps or upgrade RAM',
            });
            crashActiveRef.current = true;
          }
        } else {
        ...
        if (ssdUsageRef.current >= 100 && hddUsageRef.current >= 100) {
          ...
          if (timers.storage >= 2000) {
            setCrashEvent({
              type: 'disk_full',
              title: 'Storage Full',
              desc: 'Both SSD and HDD are full — cannot write data',
              fix: 'Clean up files or add more storage',
            });
            crashActiveRef.current = true;
          }
        } else {
        ...
        if (ramTotal > 0) {
          const appRamTotal = runningAppsRef.current.reduce(
            (s, a) => s + (a.active ? a.ramCost : 0),
            0,
          );
          if (appRamTotal > ramTotal) {
            ...
            if (timers.oom >= 2000) {
              setCrashEvent({
                type: 'bsod',
                title: 'Out of Memory (BSOD)',
                desc: 'Running apps demand more RAM than installed — critical error',
                fix: 'Close memory-heavy apps or install more RAM',
              });
              crashActiveRef.current = true;
            }
          } else {
        ...
        if (bn && bn.severity === 'severe' && bn.type !== 'thermal') {
          ...
          if (timers.bottleneck >= 5000) {
            setCrashEvent({
              type: 'slow',
              title: 'Severe Bottleneck',
              desc: 'A component is maxed out — system is extremely slow',
              fix: 'Upgrade the bottlenecked component',
            });
            crashActiveRef.current = true;
          }
        } else {

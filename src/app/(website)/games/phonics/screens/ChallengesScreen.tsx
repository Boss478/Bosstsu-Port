...
interface ChallengesScreenProps {
  onLaunch: (type: "phoneme-match" | "sound-sort" | "rhyme-time" | "speed-spell" | "syllable-smash", difficulty: "easy" | "medium" | "hard", level: CefrLevel) => void;
}
    ...
    <div className="flex-1 overflow-y-auto overscroll-contain bg-transparent min-h-full">
      <div className="max-w-md mx-auto px-6 py-8 pb-36 text-center">
        <div className="mb-6">
          ...
          <p className="text-xs font-bold text-slate-500 dark:text-slate-400 mt-1 uppercase tracking-widest">Fun mini-games to sharpen your skills</p>
        </div>
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-lg font-black text-slate-800 dark:text-white" style={{ fontFamily: "var(--font-mali)" }}>
                      {challenge.title}
                    </h3>
                    <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 mt-0.5">{challenge.subtitle}</p>
                  </div>
                </div>
                ...
                {highScore > 0 && (
                  <div className="mt-3 flex items-center gap-3 text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider border-t border-slate-200/40 dark:border-slate-800/40 pt-3">
                    ...
                    <span className="flex items-center gap-1">
                      <i className="fi fi-sr-repeat text-[#2EC4B6]" /> Played: {stats?.roundsPlayed ?? 0}
                    </span>
                  </div>
                )}
                ...
                {(!stats || stats.roundsPlayed === 0) && (
                  <div className="mt-3 border-t border-slate-200/40 dark:border-slate-800/40 pt-3">
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Not played yet</span>
                  </div>
                )}

                {isExpanded && (
                  <div className="mt-4 pt-4 border-t border-slate-200/40 dark:border-slate-800/40 animate-fadeIn">
                    <p className="text-xs font-bold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wider">Select Difficulty</p>
                    <div className="flex gap-2">
                      {(["easy", "medium", "hard"] as const).map((d) => (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            onLaunch(challenge.id as any, d, level);
                          }}
                          ...
                          style={{
                            backgroundColor: d === "easy" ? "#2EC4B6" : d === "medium" ? "#FFBA08" : "#E74C3C",
                            borderColor: d === "easy" ? "#1a8a7e" : d === "medium" ? "#d49a00" : "#c0392b",
                            color: "#fff",
                          }}
                        >
                          {d}

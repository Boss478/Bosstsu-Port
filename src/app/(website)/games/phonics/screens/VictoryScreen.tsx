...
function StatTile({ label, value, icon, color }: { label: string; value: string; icon: React.ReactNode; color: string }) {
  return (
    <div className="glass-panel p-4 rounded-2xl flex flex-col items-center justify-center border border-white/20 shadow-xs hover:-translate-y-1 transition-transform duration-300">
      <span className="text-xl mb-1 filter drop-shadow-2xs select-none flex items-center justify-center" style={{ color }}>{icon}</span>
      <span className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-0.5">{label}</span>
      <span className="text-lg font-extrabold" style={{ color }}>{value}</span>
    </div>
  );
}

export default function VictoryScreen({ round, onPlayAgain, onRetryIncorrect, onBackToPath }: VictoryScreenProps) {
  const { save } = useGame();
      ...
      <div className="flex flex-col items-center justify-center px-6 py-10 pb-28 max-w-md mx-auto w-full relative z-10">
        ...
        {round.config.isPlacement ? (
          <div className="w-full text-center space-y-6">
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold text-slate-800 dark:text-[#F7E1A0] tracking-wider uppercase drop-shadow-sm" style={{ fontFamily: "var(--font-mali)" }}>
              YOUR PLACEMENT
            </h1>
            ...
            <div className="glass-panel p-6 rounded-3xl border-2 border-[#C8A44E]/40 shadow-xl max-w-sm mx-auto bg-gradient-to-br from-white/80 to-[#FAE8FF]/30 dark:from-slate-900/80 dark:to-[#2A1242]/30 relative overflow-hidden flex flex-col items-center">
              <span className="text-5xl md:text-6xl filter drop-shadow-sm select-none mb-3">🎓</span>
              <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Target Proficiency Level</p>
              <h2 className="text-2xl md:text-3xl font-black text-[#C8A44E] mt-1" style={{ fontFamily: "var(--font-mali)" }}>
              ...
              <p className="text-xs text-slate-600 dark:text-slate-350 mt-4 font-medium leading-relaxed px-4">
                {save?.cefrLevel === "a1" && "Placed at Beginner Level. We will start with basic short vowel sounds and high-frequency phonemes."}
                {save?.cefrLevel === "a2" && "Placed at Elementary Level. We will practice short vowel combinations and initial blends."}
                {save?.cefrLevel === "b1" && "Placed at Intermediate Level. Focus is on long vowels, diphthongs, and common consonant digraphs."}
                {save?.cefrLevel === "b2" && "Placed at Upper-Intermediate Level. We will cover advanced diphthongs, R-controlled vowels, and complex syllable structures."}
                {save?.cefrLevel === "c1" && "Placed at Advanced Level. Focus is on rare phoneme segments and complex words."}
                {save?.cefrLevel === "c2" && "Placed at Mastery Level. Challenging vocabulary and rapid speed rounds."}
              </p>
            </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 gap-3.5 w-full mt-6">
              <StatTile label="Score" value={`${round.score}`} icon={<i className="fi fi-sr-star" />} color="#C8A44E" />
              <StatTile label="Accuracy" value={`${accuracy}%`} icon={<i className="fi fi-sr-medal" />} color="#2EC4B6" />
              <StatTile label="Best Streak" value={`${round.maxStreak}`} icon={<i className="fi fi-sr-flame" />} color="#FFBA08" />
              <StatTile label="Coins" value={`+${round.coinsEarned}`} icon={<i className="fi fi-sr-wallet" />} color="#C8A44E" />
            </div>
            ...
            {save && (
              <div className="w-full text-center mt-3 text-xs font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest flex items-center gap-1.5 justify-center">
                Total Inventory: <span className="text-[#C8A44E] flex items-center gap-1"><i className="fi fi-sr-wallet text-xs" /> {save.phonemeCoins}</span>
              </div>
            )}
          </>
        )}
        ...
        {levelChange === "downgrade" && (
          <div className="w-full max-w-sm p-4 mt-6 rounded-3xl border bg-amber-500/10 dark:bg-amber-950/20 border-amber-400/40 text-amber-600 dark:text-amber-400 font-extrabold text-xs tracking-wider uppercase flex items-center justify-center gap-2.5">
            ...
            <span>Level Adjusted: set to {CEFR_LEVEL_LABELS[save?.cefrLevel ?? "a1"]} to match performance.</span>
          </div>
        )}
        ...
        <div className="glass-panel rounded-3xl border border-white/20 shadow-sm w-full mt-6 overflow-hidden">
          <div className="p-4 border-b border-white/20 dark:border-slate-800 flex items-center justify-between bg-white/30 dark:bg-slate-900/20">
            <p className="text-xs font-bold text-slate-700 dark:text-slate-200 tracking-widest uppercase">QUESTION REVIEW</p>
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 px-4 py-3 text-left transition-colors cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 ${
                    r.correct ? "bg-emerald-500/5 dark:bg-emerald-950/5" : "bg-rose-500/5 dark:bg-rose-950/5"
                  }`}
                  onClick={() => wordData && setSelectedReviewWord(wordData)}
                >
                  <span className={`text-base font-bold ${r.correct ? "text-emerald-500" : "text-rose-500"}`}>
                    {r.correct ? "✓" : "✗"}
                  </span>
                  
                  <div className="flex-1 min-w-0">
                    <p className="text-xs font-bold text-slate-700 dark:text-slate-200 truncate">
                      {r.question.category === "phonics" && r.question.format !== "card-flip" && phonemeIpa
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    <button 
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        wordData && setSelectedReviewWord(wordData);
                      }}
                    >
        ...
        {selectedReviewWord && (
          ...
          >
            <div 
              className="w-full max-w-sm rounded-3xl glass-heavy border border-white/60 dark:border-slate-800 p-6 shadow-2xl space-y-4 animate-scale-up text-left"
              onClick={(e) => e.stopPropagation()}
            >
              {/* Header info */}
              <div className="flex justify-between items-start">
                <div>
                  <h3 className="text-2xl font-black text-slate-800 dark:text-white tracking-wide leading-tight" style={{ fontFamily: "var(--font-mali)" }}>
              ...
              <div className="flex items-center gap-3 bg-[#C8A44E]/10 border border-[#C8A44E]/20 rounded-2xl p-3">
                ...
                <div>
                  <p className="text-xs font-extrabold text-[#C8A44E] tracking-widest uppercase">Listen</p>
                  <p className="text-[10px] text-slate-500 dark:text-slate-400 font-medium">Tap to hear correct pronunciation & stress</p>
                </div>
              </div>
              ...
              <div className="space-y-3 pt-1">
                <div className="space-y-1">
                  <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Definition</p>
                ...
                {selectedReviewWord.example && (
                  <div className="space-y-1 border-t border-slate-100 dark:border-slate-800/60 pt-2.5">
                    <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Example</p>
              ...
              {selectedReviewWord.syllables && selectedReviewWord.syllables.length > 0 && (
                <div className="border-t border-slate-100 dark:border-slate-800/60 pt-2.5">
                  <p className="text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1.5">Syllables</p>
                  <div className="flex gap-1">
                    {selectedReviewWord.syllables.map((s, idx) => (
                      ...
                      >
                        {s}
                        {selectedReviewWord.stress && selectedReviewWord.stress.includes(idx) && " ˈ"}
                      </span>
                    ))}
                  </div>
                </div>

...
export default function WordBuilderScreen() {
  ...
  const [layoutMode, setLayoutMode] = useLocalStorage<"vertical" | "horizontal">("word-builder-layout", "horizontal");
  const [zoomLevel, setZoomLevel] = useLocalStorage<number>("word-builder-zoom", 100);
  const [keyboardLayout, setKeyboardLayout] = useLocalStorage<"qwerty" | "alphabetical">("word-builder-keyboard-layout", "qwerty");
  const [dialectPreference, setDialectPreference] = useLocalStorage<"both" | "us" | "uk">("word-builder-dialect", "both");
  const [tapSoundsEnabled, setTapSoundsEnabled] = useLocalStorage<boolean>("word-builder-tap-sounds", true);
    ...
    <div className="flex-1 flex flex-col overflow-hidden min-h-0">
      <div className="shrink-0 px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-extrabold text-slate-800 dark:text-[#F7E1A0] tracking-wide" style={{ fontFamily: "var(--font-mali)" }}>
            Word Builder
          </h1>
        </div>
      ...
      {isSettingsOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 animate-fade-in">
          <div 
            className="w-full max-w-sm bg-white/95 dark:bg-slate-900/95 border border-white/50 dark:border-slate-800/50 rounded-3xl p-6 shadow-2xl space-y-5 transform scale-100 transition-transform duration-200"
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  >
                    ...
                    <span className="text-[9px] text-slate-400 dark:text-slate-500 mt-1">Split columns on desktop</span>
                  </button>
                  ...
                  >
                    ...
                    <span className="text-[9px] text-slate-400 dark:text-slate-500 mt-1">Stacked top-to-bottom</span>
                  </button>
                </div>
              </div>
                ...
                <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-white/50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50">
                  ...
                  <div className="flex-1 text-center select-none">
                    ...
                    <span className="text-[8px] text-slate-400 dark:text-slate-500 uppercase font-bold mt-0.5 block tracking-wider">
                      {zoomLevel === 100 ? "Default size" : zoomLevel < 100 ? "Compact view" : "Enlarged view"}
                    </span>
                  </div>
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(["both", "ipa", "example"] as const).map((mode) => {
                    const label = mode === "both" ? "Both" : mode === "ipa" ? "IPA Only" : "Word Only";
...
function SpellingToIpaTab({
    ...
    if (typeof window !== "undefined") {
      ...
      if (stored) {
        try {
          ...
          if (Array.isArray(list)) return list.map(w => typeof w === "string" ? w.toUpperCase() : "");
        } catch {}
      }
    }
    return [];
  ...
  const addToHistory = useCallback((word: string) => {
    setSearchHistory((prev) => {
      ...
      const next = [cleaned, ...prev.filter((w) => w.toUpperCase() !== cleaned.toUpperCase())].slice(0, 10);
      if (typeof window !== "undefined") {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      setNetworkError(false);
      try {
        const res = await fetch(`/api/dictionary?word=${encodeURIComponent(q)}&fields=ipa,definition,word`);
        if (cancelled) return;
        ...
        <div className={`${layoutMode === "horizontal" ? "lg:col-span-5 min-h-[380px]" : "w-full min-h-0"} ${WB_PANEL_BASE} space-y-4 flex flex-col justify-start`}>
          <div className="relative">
            ...
            <input
              ...
              type="text"
              value={searchText}
              onChange={(e) => { setSearchText(e.target.value); setNetworkError(false); }}
...
function IpaToWordTab({
  ...
  const selectedWordData = useMemo(() => {
    ...
    return allWordEntries.find(w => w.word.toLowerCase() === selectedWordName.toLowerCase()) || null;
  ...
  const autoSelectPhonemes = useCallback((w: typeof allWordEntries[number]) => {
    playTapSound();
            ...
            {selectedPhonemes.length === 0 ? (
              ...
              <>
                <div className="flex-1 min-w-0">
                  <span className="text-sm font-black text-slate-800 dark:text-white" style={{ fontFamily: "var(--font-geist-mono)" }}>
                    {combinedIpa}
                  </span>
                </div>

...
const MODES: { value: QuizMode; label: string; desc: string; icon: string }[] = [
  ...
  },
  {
    value: 'endless',
    label: 'Endless',
    desc: 'Keep going until you run out of lives',
    icon: 'fi-sr-infinity',
  },
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {DIRECTIONS.map((d) => (
                ...
                >
                  <span
                    className={`text-[10px] font-extrabold block ${direction === d.value ? 'text-[#C8A44E]' : 'text-slate-700 dark:text-slate-200'}`}
                  >
                    {d.label}
                  </span>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {MODES.map((m) => (
                ...
                >
                  <i
                    className={`fi ${m.icon} text-sm ${mode === m.value ? 'text-[#C8A44E]' : 'text-slate-500 dark:text-slate-400'}`}
                  />
                  <span
                    className={`text-[10px] font-extrabold block mt-1 ${mode === m.value ? 'text-[#C8A44E]' : 'text-slate-700 dark:text-slate-200'}`}
                  >
                    {m.label}
                  </span>
                  <span className="text-[7px] text-slate-400 dark:text-slate-500 mt-0.5 block">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                <span
                  className={`text-[10px] font-extrabold block ${difficulty === 'normal' ? 'text-[#C8A44E]' : 'text-slate-700 dark:text-slate-200'}`}
                >
                  Normal
                </span>
                <span className="text-[7px] text-slate-400 dark:text-slate-500 mt-0.5 block">
              ...
              >
                <span
                  className={`text-[10px] font-extrabold block ${difficulty === 'difficult' ? 'text-[#C8A44E]' : 'text-slate-700 dark:text-slate-200'}`}
                >
                  Difficult
                </span>
                <span className="text-[7px] text-slate-400 dark:text-slate-500 mt-0.5 block">

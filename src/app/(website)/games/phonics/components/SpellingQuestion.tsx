...
export default function SpellingQuestion({
  ...
  const choices =
    question.choices ?? [question.word.word, ...question.word.spellingDistractors].slice(0, 4);

  return (
    <div className="flex flex-col items-center justify-center px-4 py-6 text-center">
      <p className="text-xs font-bold text-[#1C1C1C]/40 dark:text-[#F7E1A0]/40 tracking-widest mb-3 uppercase">
        SPELL THE WORD
      </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full max-w-4xl">
        {choices.map((option, i) => {
          ...
          let btnStyle =
            'w-full px-5 py-4 font-bold text-sm tracking-wide text-center rounded-2xl glass-elem border-2 border-white/60 dark:border-slate-700/50 text-[#1C1C1C] dark:text-[#F7E1A0] hover:bg-white/80 dark:hover:bg-slate-700/80 btn-3d shadow-sm cursor-pointer ';
          let borderStyle = { '--border-color': 'rgba(0,0,0,0.12)' } as React.CSSProperties;

          if (feedback === 'correct' && isCorrect) {
            btnStyle +=
              'bg-emerald-500 text-white border-emerald-500 hover:bg-emerald-500 dark:border-emerald-500 animate-correct-bounce';
            borderStyle = { '--border-color': '#1b8a5a' } as React.CSSProperties;
          } else if (feedback === 'wrong' && isCorrect) {
            btnStyle +=
              'bg-emerald-500/40 text-emerald-800 dark:text-emerald-200 border-emerald-500/40';
            borderStyle = { '--border-color': 'transparent' } as React.CSSProperties;
          } else if (feedback === 'wrong' && isSelected && !isCorrect) {
            btnStyle +=
              'bg-rose-500 text-white border-rose-500 hover:bg-rose-500 dark:border-rose-500 animate-shake';
      ...
      {companionData && feedback && hintCount < 3 && (
        <div
          className="mt-6 p-4 rounded-2xl text-sm text-left max-w-sm glass-panel border-l-4"
          style={{ borderLeftColor: companionData.color }}
        >
          <span className="font-bold" style={{ color: companionData.color }}>
            {companionData.name}:{' '}
          </span>
...
function TilesSpelling({
  ...
  return (
    <div className="flex flex-col items-center justify-center px-4 py-6 text-center">
      ...
      <div
        className={`w-full max-w-4xl min-h-[72px] p-4 mb-4 rounded-3xl glass-panel flex flex-wrap justify-center items-center gap-2 border-2 transition-colors duration-300 ${
          feedback === 'correct'
      ...
      <div className="flex flex-wrap justify-center gap-2.5 mb-6 max-w-4xl">
        {shuffledTiles.map((phoneme, i) => {
          ...
          const used =
            selectedTiles.filter((t) => t === phoneme).length >
      ...
      {companionData && feedback && hintCount < 3 && (
        <div
          className="mt-2 p-4 rounded-2xl text-sm text-left max-w-sm glass-panel border-l-4"
          style={{ borderLeftColor: companionData.color }}
        >
          <span className="font-bold" style={{ color: companionData.color }}>
            {companionData.name}:{' '}

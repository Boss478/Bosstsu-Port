...
const HEAD_SPRITES: Record<string, SpriteData> = {
  nox: AVATAR_NOX_HEAD,
  mira: AVATAR_MIRA_HEAD,
  chip: AVATAR_CHIP_HEAD,
  fox: AVATAR_FOX_HEAD,
  cat: AVATAR_CAT_HEAD,
  bear: AVATAR_BEAR_HEAD,
  bunny: AVATAR_BUNNY_HEAD,
  penguin: AVATAR_PENGUIN_HEAD,
  alien: AVATAR_ALIEN_HEAD,
  ninja: AVATAR_NINJA_HEAD,
...
export default function MascotCanvas({
  ...
  const spriteData =
    (variant === 'full' ? FULL_SPRITES : HEAD_SPRITES)[companionId] ??
    (variant === 'full' ? FULL_SPRITES : HEAD_SPRITES).nox;
  const config = useMemo(
    () =>
      variant === 'head' ? (HEAD_CONFIG[companionId] ?? HEAD_CONFIG.nox) : { x: 0, y: 0, scale: 1 },
    [variant, companionId],
  );
  ...
  useEffect(() => {
    ...
    const sprite =
      blinking && variant === 'head' ? blinkSprite(spriteData, companionId) : spriteData;
    ...
    if (accId) {
      ...
      if (accSprite) {
        const accY =
          variant === 'head' ? config.y + (HEAD_ACC_Y_OFFSET[companionId] ?? 0) : config.y;

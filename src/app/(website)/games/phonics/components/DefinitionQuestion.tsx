    ...
    <div className="flex flex-col items-center justify-center px-4 py-6 text-center">
      {isDefToWord ? (
        <div className="w-full max-w-4xl mb-6">
          <p className="text-xs font-bold text-[#1C1C1C]/40 dark:text-[#F7E1A0]/40 tracking-widest mb-3 uppercase">
            {question.blankedExample ? 'GAP-FILL' : 'DEFINITION'}
          </p>
      ...
      <div className="grid grid-cols-1 gap-5 w-full max-w-4xl">
        {question.options.map((option, i) => {
          ...
          let btnStyle =
            'w-full px-5 py-4 font-bold text-sm tracking-wide text-center rounded-2xl backdrop-blur-xs border-2 border-white/60 dark:border-slate-700/50 btn-3d shadow-sm cursor-pointer ';
          ...
          if (feedback === 'correct' && isCorrect) {
            ...
            btnStyle +=
              'border-[#9B59B6] bg-[#9B59B6]/10 dark:bg-[#9B59B6]/20 text-[#9B59B6] dark:text-[#F7E1A0]';
            borderStyle = { '--border-color': '#7e429c' } as React.CSSProperties;
          } else {
            btnStyle +=
              'bg-white/60 dark:bg-slate-800/60 text-[#1C1C1C] dark:text-[#F7E1A0] hover:bg-white/80 dark:hover:bg-slate-700/80';
          }
      ...
      {companionData && feedback && hintCount < 3 && (
        <div
          className="mt-4 p-4 rounded-2xl text-sm text-left max-w-sm glass-panel border-l-4"

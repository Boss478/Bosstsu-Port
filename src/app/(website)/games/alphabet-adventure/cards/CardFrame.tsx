...
const TIER_BACK = {
  common: { bg: "#d4d4d8", accent: "#a1a1aa", emblem: "M 80 180 L 100 140 L 120 180 Z M 80 180 L 60 160 L 100 140 M 120 180 L 140 160 L 100 140", symbol: "⚔" },
  uncommon: { bg: "#bbf7d0", accent: "#22c55e", emblem: "M 100 140 Q 80 160 85 180 Q 100 190 100 190 Q 100 190 115 180 Q 120 160 100 140 Z", symbol: "🍀" },
  rare: { bg: "#bfdbfe", accent: "#3b82f6", emblem: "M 100 140 L 105 155 L 120 155 L 108 165 L 112 180 L 100 170 L 88 180 L 92 165 L 80 155 L 95 155 Z", symbol: "✦" },
  "ultra-rare": { bg: "#d8b4fe", accent: "#a855f7", emblem: "M 100 135 Q 110 150 125 145 Q 115 158 120 175 Q 100 165 80 175 Q 85 158 75 145 Q 90 150 100 135 Z M 100 150 L 103 158 L 112 160 L 105 165 L 107 175 L 100 168 L 93 175 L 95 165 L 88 160 L 97 158 Z", symbol: "⭐" },
  legendary: { bg: "#fde68a", accent: "#f59e0b", emblem: "M 85 170 L 100 140 L 115 170 L 140 170 L 120 185 L 128 210 L 100 192 L 72 210 L 80 185 L 60 170 Z", symbol: "👑" },
};
...
export function CardFrame({ tier, children, namePlate, size = "album", showBack, holographic = false }: CardFrameProps) {
      ...
      >
        ...
        {showBack ? (
          <>
            ...
            {/* Question mark */}
            <text x="100" y="150" textAnchor="middle" dominantBaseline="central"
              fontSize="60" fontWeight="900" fill={back!.accent} opacity="0.2"
          ...
          <>
            ...
            <rect x="3" y="3" width="194" height="274" rx="13"
              stroke={`url(#border-${tier})`} strokeWidth="4" fill="none"
              filter={`url(#distress-${tier})`} />

            {/* Inner border line (clean, no filter) */}
            <rect x="9" y="9" width="182" height="262" rx="10"
              stroke={frame.primary} strokeWidth="1" fill="none" opacity="0.3" />

            {/* Portrait window arch */}
            <path d="M 12 42 L 12 185 L 188 185 L 188 42 Q 188 14 100 14 Q 12 14 12 42 Z"
              fill="none" stroke={frame.primary} strokeWidth="1.5" opacity="0.4" />
            ...
            {/* Name plate */}
            <rect x="20" y={size === "toast" ? 202 : 210} width={size === "toast" ? 160 : 160} height={size === "toast" ? 22 : 26} rx="6"
              fill={`url(#plate-${tier})`} stroke={frame.primary} strokeWidth="1" />

            {/* Plate text */}
            {namePlate && (
              <text x="100" y={size === "toast" ? 215 : 225}
                textAnchor="middle" dominantBaseline="central"
                fontSize={size === "toast" ? "8" : size === "modal" ? "11" : "9"}
                fontWeight="800" fill={frame.plateText} letterSpacing="1"
                style={{ userSelect: "none" }}>
                {namePlate}
              </text>
            )}
            ...
            {/* Tier gem */}
            <circle cx={gemCx} cy={gemCy} r={size === "toast" ? "5" : size === "modal" ? "10" : "7"}
              fill={frame.gem} stroke={frame.secondary} strokeWidth="1" />
          </>
        )}
      </svg>
      ...
      {holographic && !showBack && (
        <div className="absolute inset-0 overflow-hidden rounded-[13px] pointer-events-none">
          <div
            ...
            style={{
              background: "linear-gradient(135deg, transparent 0%, rgba(255,255,255,0.6) 40%, rgba(255,255,255,0.8) 50%, rgba(255,255,255,0.6) 60%, transparent 100%)",

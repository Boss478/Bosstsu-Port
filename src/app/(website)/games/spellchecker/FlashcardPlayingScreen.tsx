...
export default function FlashcardPlayingScreen() {
  ...
  const finalizeMouseDrag = () => {
    if (isAnimating || mouseStartXRef.current === null || feedbackHint) return;
      ...
      <div className="w-full max-w-2xl h-full flex flex-col items-center justify-between relative">
        ...
        <div className="w-full flex justify-between items-center mb-4 px-4 h-10 shrink-0">
          ...
          <div className="flex items-center gap-4 font-bold">
            {mode === 'TIMER' && (
              <div
                className={`flex items-center gap-2 ${timeLeft <= 10 ? 'text-red-500 animate-pulse' : 'text-zinc-800 dark:text-zinc-200'}`}
              >
            ...
            {mode !== 'TIMER' && (
              <div className="flex items-center gap-2 text-zinc-500 dark:text-zinc-400">
                <i aria-hidden="true" className="fi fi-sr-stopwatch"></i>{' '}
            ...
            {mode === 'LIFE' && (
              <div className="flex gap-1 text-red-500">
                {Array.from({ length: 3 }).map((_, i) => (
                  <i
                    key={i}
                    className={`fi ${i < lives ? 'fi-sr-heart' : 'fi-rr-heart text-zinc-300 dark:text-zinc-700'}`}
                  ></i>
                ))}
              </div>
            )}
        ...
        <div className="flex-1 w-full flex items-center justify-center min-h-0 relative z-10 sm:mb-6">
          {feedback && !feedbackHint && (
            <div className="fixed inset-x-0 top-1/2 -translate-y-1/2 z-200 flex justify-center pointer-events-none">
              <div
                className={`text-3xl sm:text-4xl font-bold font-sans tracking-wide animate-float-up-fade drop-shadow-lg flex items-center gap-2 whitespace-nowrap ${feedback === 'CORRECT' ? 'text-emerald-500' : 'text-rose-500'}`}
              >
                {feedback === 'CORRECT' ? (
                  <>
          ...
          <div className="relative w-full max-w-sm aspect-4/3 perspective-1000 select-none group focus:outline-none">
            <div
              ...
              style={{
                ...
                borderColor:
                  swipeOffset < -50 ? '#10b981' : swipeOffset > 50 ? '#ef4444' : 'transparent',
              ...
              onMouseMove={(e: React.MouseEvent) => {
                if (isAnimating || mouseStartXRef.current === null || feedbackHint) return;
                setSwipeOffset(e.clientX - mouseStartXRef.current);
              }}
              onMouseUp={finalizeMouseDrag}
              onMouseLeave={finalizeMouseDrag}
            >
              <h2
                className={`text-4xl sm:text-5xl font-bold tracking-wide text-zinc-800 dark:text-zinc-100 text-center pointer-events-none ${feedbackHint ? 'opacity-30' : ''}`}
              >
                {currentWord.word}
              </h2>
              ...
              {currentWord &&
                ...
                currentWord.isCorrect && (
                  <div
                    className={`flex items-center gap-2 mt-4 pointer-events-none transition-opacity ${feedbackHint ? 'opacity-30' : 'opacity-100'}`}
                  >
              ...
              <div
                className="absolute top-4 left-4 text-emerald-500 font-bold text-2xl pointer-events-none"
                style={{ opacity: swipeOffset < -50 ? (-swipeOffset - 50) / 100 : 0 }}
              >
                CORRECT
              </div>
              <div
                className="absolute top-4 right-4 text-rose-500 font-bold text-2xl pointer-events-none"
                style={{ opacity: swipeOffset > 50 ? (swipeOffset - 50) / 100 : 0 }}
              >
                WRONG
              </div>
              ...
              {feedbackHint && (
                <div className="absolute inset-0 bg-black/10 rounded-2xl flex flex-col items-center justify-center p-6 animate-in fade-in">
                  <i
                    aria-hidden="true"
                    className="fi fi-sr-cross-circle text-5xl text-rose-500 mb-4 drop-shadow-sm"
                  ></i>
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    {currentWord && currentWord.definition && currentWord.isCorrect && (
                      <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-2 px-4 text-center max-w-sm">
                        <span className="font-bold text-blue-500">Def: </span>
        ...
        <div className="w-full shrink-0 flex flex-col items-center">
          ...
          <div className="text-zinc-400 mt-4 text-sm flex gap-6">
            <span className="flex items-center">
              <kbd className="mr-2 hidden sm:inline-block">←</kbd> Correct
            </span>
            <span className="flex items-center">
              <kbd className="mr-2 hidden sm:inline-block">→</kbd> Incorrect
            </span>
          </div>
        </div>
      </div>

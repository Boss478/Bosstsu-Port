...
export default function PhotoLightbox({ photos, initialIndex, onClose }: PhotoLightboxProps) {
  ...
  const [exifCache, setExifCache] = useState<Map<string, ExifData>>(() => new Map());
  ...
  const effectiveExif = cached ?? exifData;
  const effectiveLoading = exifLoading && !cached;
  const effectiveFileInfo = cached ? { name: filename, size: '' } : fileInfo;
  ...
  useEffect(() => {
    ...
    import('exifr')
      ...
      .then((data) => {
        ...
        // Cache the result
        setExifCache((prev) => new Map(prev).set(url, parsed));
        setExifData(parsed);
        setExifLoading(false);
      })
      .catch(() => {
  ...
  const hasExif = effectiveExif !== null && Object.values(effectiveExif).some(Boolean);

  const infoRows: { icon: string; label: string; value: string }[] = [];
  if (effectiveExif?.camera)
    infoRows.push({ icon: 'fi fi-sr-camera', label: 'Camera', value: effectiveExif.camera });
  if (effectiveExif?.lens)
    infoRows.push({ icon: 'fi fi-sr-circle', label: 'Lens', value: effectiveExif.lens });
  if (effectiveExif?.focalLength)
    infoRows.push({
      icon: 'fi fi-sr-search',
      label: 'Focal Length',
      value: effectiveExif.focalLength,
    });
  if (effectiveExif?.aperture)
    infoRows.push({ icon: 'fi fi-sr-aperture', label: 'Aperture', value: effectiveExif.aperture });
  if (effectiveExif?.shutter)
    infoRows.push({ icon: 'fi fi-sr-stopwatch', label: 'Shutter', value: effectiveExif.shutter });
  if (effectiveExif?.iso)
    infoRows.push({ icon: 'fi fi-sr-brightness', label: 'ISO', value: effectiveExif.iso });
  if (effectiveExif?.dateTaken)
    infoRows.push({
      icon: 'fi fi-sr-calendar',
      label: 'Date Taken',
      value: effectiveExif.dateTaken,
    });
          ...
          <div className="mt-4 md:mt-0 md:absolute md:bottom-0 md:left-full md:ml-3 z-20 flex flex-col gap-1.5 w-[95vw] sm:w-[80vw] md:w-[200px] lg:w-[240px] animate-fade-in-up origin-top md:origin-bottom-left">
            ...
            {effectiveFileInfo && (
              <div className="w-full bg-black/60 backdrop-blur-sm border border-white/10 px-3 py-2 rounded-xl text-[10px] font-mono shadow-lg flex flex-row md:flex-col items-center md:items-start justify-between md:justify-start gap-2 md:gap-0.5 box-border">
                <span className="font-medium text-white break-all text-left">
                  {effectiveFileInfo.name}
                </span>
                <span className="text-white/50 shrink-0 whitespace-nowrap">
                  {effectiveFileInfo.size}
                </span>
              </div>
            )}
                    ...
                    {infoRows.map((row) => (
                      <div key={row.label} className="flex items-start gap-2">
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="text-[8px] text-white/50 uppercase tracking-wider mb-px">
                            {row.label}
                          </p>
                          <p className="text-[10px] sm:text-xs font-medium text-white/90 leading-tight wrap-break-word">
                            {row.value}
                          </p>
                        </div>
                      </div>

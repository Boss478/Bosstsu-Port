...
export default function PadletBoard({
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (stored) {
        try {
          startTransition(() => setOwnPosts(JSON.parse(stored)));
        } catch {
          ...
          {posts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {posts.map((post) => (
                <div
                  key={post._id}
                  className="animate-fade-slide-up p-4 rounded-xl bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border border-white/60 dark:border-slate-700/50 shadow-sm"
                >

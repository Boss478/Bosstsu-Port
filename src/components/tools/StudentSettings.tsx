...
export default function StudentSettings({
  ...
  const handleSettingChange = useCallback(
    <K extends keyof TierConfig>(key: K, value: TierConfig[K]) => {
      setOverrides((prev) => {
  ...
  const effectsLevel = useMemo(() => {
    if (
      isOverridden('transitions') ||
      isOverridden('particles') ||
      isOverridden('hoverEffects') ||
      isOverridden('shadows') ||
      isOverridden('gradients') ||
      isOverridden('skeleton')
    ) {
  ...
  const effectsOverridden = [
    ...
    'gradients',
    'skeleton',
  ].some((k) => isOverridden(k as keyof TierConfig));
    ...
    <>
      ...
      >
        <svg
          ...
          className="w-4 h-4 shrink-0 text-zinc-600 dark:text-zinc-300"
        >
              ...
              {FILTERS.map((f) => {
                ...
                return (
                  <button
                    ...
                    title={
                      f.label === 'ALL' ? 'Show all settings' : `Show ${f.label} settings only`
                    }
            ...
            {sectionFilter === 'all' && onMascotSelect && (
              <div className="space-y-2">
                ...
                <div className="max-h-48 overflow-y-auto scrollbar-thin">
                  <MascotSelector
                    selectedId={selectedMascot ?? null}
                    onSelect={(id) => {
                      onMascotSelect(id);
                      setOpen(false);
                    }}
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2 text-xs text-zinc-400">
                <span>
                  ...
                  <span className="font-medium text-zinc-600 dark:text-zinc-300 ml-1 uppercase">
                    {TIER_LABELS[tier]}
                  </span>
                </span>
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          handleSettingChange(
                            'imageQuality',
                            Number(e.target.value) as TierConfig['imageQuality'],
                          )
                        }
                    ...
                    <div className="flex items-center gap-2">
                      <input
                        ...
                        onChange={(e) =>
                          handleSettingChange(
                            'backdropBlur',
                            blurPercentToPx(Number(e.target.value)) as TierConfig['backdropBlur'],
                          )
                        }
                      ...
                      />
                      <span className="text-xs text-zinc-500 w-7 text-right">
                        {blurPxToPercent(effectiveConfig.backdropBlur)}%
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-zinc-50 dark:bg-slate-900/50">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleSettingChange('fps', Number(e.target.value) as TierConfig['fps'])
                      }
                  ...
                  <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-zinc-50 dark:bg-slate-900/50">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        handleSettingChange(
                          'pollIntervalMs',
                          Number(e.target.value) as TierConfig['pollIntervalMs'],
                        )
                      }
              ...
              <div className="space-y-2 pt-1">
                ...
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-zinc-50 dark:bg-slate-900/50">
                    ...
                    >
                      <div
                        className={`w-4 h-4 rounded-full bg-white absolute top-0.5 transition-transform ${soundOn ? 'translate-x-5' : 'translate-x-0.5'}`}
                      />
                    </div>
                  </div>
                  <div className="flex items-center justify-between px-3 py-2 rounded-xl bg-zinc-50 dark:bg-slate-900/50">
                    ...
                    <div
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter' || e.key === ' ') toggleSoundtrack();
                      }}
                    ...
                    >
                      <div
                        className={`w-4 h-4 rounded-full bg-white absolute top-0.5 transition-transform ${soundtrackOn ? 'translate-x-5' : 'translate-x-0.5'}`}
                      />
                    </div>
                  </div>
                </div>

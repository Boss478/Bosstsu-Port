...
export default function QuickQuiz({ session, stepIndex, studentName, mascot, onMascotEvent }: QuickQuizProps) {
  ...
  const maxSubmissions = (resolvedStepCfg?.maxSubmissions as number | undefined) ?? session.config?.maxSubmissions ?? 1;
  ...
  useEffect(() => {
    ...
    if (!token) return;
    const myResponses = (pollData.responses || []).filter((r: { studentToken?: string }) => r.studentToken === token);
        ...
        <div className="max-w-lg w-full space-y-4">
          ...
          {history.length > 0 && (
            <div className="p-6 rounded-2xl bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border border-white/60 dark:border-slate-700/50 shadow-sm">
              <h3 className="text-sm font-semibold text-zinc-500 dark:text-zinc-400 uppercase tracking-wide mb-3">{t('attemptHistory')}</h3>
              <div className="space-y-2">
                {history.map((h, i) => (
                  <div key={i} className="flex items-center justify-between py-2 px-3 rounded-lg bg-zinc-50 dark:bg-slate-900/50">
                    <span className="text-sm text-zinc-500 dark:text-zinc-400">{t('attemptLabel')} {history.length - i}</span>
                    <span className="text-sm font-bold text-zinc-900 dark:text-zinc-100">{h.score}/{totalQuestions}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="max-w-lg w-full">
        <div className="text-center mb-6">
          ...
          <div className="flex items-center justify-center gap-3 text-sm text-zinc-500">
            ...
            <div className="w-32 h-2 rounded-full bg-zinc-200 dark:bg-slate-700 overflow-hidden">
              <div className="h-full bg-blue-500 transition-all" style={{ width: `${((currentQ + 1) / total) * 100}%` }} />
            </div>
          </div>
          {maxSubmissions > 0 && (
        ...
        {currentQuestion && (
          <div className="p-6 rounded-2xl bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border border-white/60 dark:border-slate-700/50 shadow-sm space-y-4 h-[360px] overflow-y-auto">
            <p className="text-lg font-bold text-zinc-900 dark:text-zinc-100">{currentQuestion.question}</p>
            <div className="space-y-2">
        ...
        <div className="flex gap-3 mt-4">
          ...
          {currentQ < total - 1 ? (
            ...
            >{submitMutation.isPending ? t('submitting') : t('submitQuiz')}</button>
          )}
        </div>
      </div>
    </div>

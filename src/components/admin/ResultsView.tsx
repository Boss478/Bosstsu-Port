...
export default function ResultsView({
  ...
  const steps = session.steps as
    Array<{ type: string; title: string; config?: Record<string, unknown> }> | undefined;
  ...
  const stepSession =
    hasSteps && activeStepTab >= 0 && steps && steps[activeStepTab].config
    ...
    startTransition(() => {
      setResponses((prev) => {
        if (stepIdx !== undefined && hasSteps) {
          const others = prev.filter((r: { stepIndex?: number }) => {
          ...
          >
            {stepRes.map(
              (r: {
                ...
                <div
                  key={r._id}
                  className="p-4 rounded-xl bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border border-white/60 dark:border-slate-700/50 shadow-sm"
                >
                  ...
                  <p className="text-zinc-700 dark:text-zinc-300 text-sm break-words">
                    {r.content?.message}
                  </p>
              ...
              <tbody>
                {stepRes.map(
                  (r: {
                    ...
                    <tr
                      key={r._id}
                      className="border-b last:border-0 border-zinc-100/60 dark:border-slate-700/30"
                    >
                      ...
                      <td className="p-4 hidden lg:table-cell text-xs text-zinc-400 font-mono">
                        {r.ip || '—'}
                      </td>
      ...
      case 'qa_board':
        return (
          <div className="space-y-3">
            {stepRes
              .sort(
                (a: { content: { upvotes?: number } }, b: { content: { upvotes?: number } }) =>
                  (b.content?.upvotes || 0) - (a.content?.upvotes || 0),
              )
              .map(
                (r: {
                  _id: string;
                  ...
                  <div
                    key={r._id}
                    className={`p-4 rounded-xl backdrop-blur-sm border shadow-sm ${
                    ...
                    <div className="flex items-start gap-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-zinc-400 mt-1">
                          {r.content?.upvotes || 0} upvote{r.content?.upvotes !== 1 ? 's' : ''} ·{' '}
                          {new Date(r.createdAt).toLocaleTimeString('th-TH')}
                        </p>
                      </div>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        <h2 className="text-xl font-bold text-zinc-900 dark:text-zinc-100 flex items-center gap-2">
          {hasSteps && activeStepTab === -1 ? (
            <>
              {t('allSteps')}{' '}
              <span className="bg-zinc-200/60 dark:bg-slate-600/60 rounded-full text-xs px-2 py-0.5 leading-none">
                {responses.length}
              </span>
            </>
          ) : activeStepType ? (
            <>
              Step {activeStepTab + 1}: {steps![activeStepTab].title}{' '}
              <span className="bg-zinc-200/60 dark:bg-slate-600/60 rounded-full text-xs px-2 py-0.5 leading-none">
                {stepCounts[activeStepTab] ?? 0}
              </span>
            </>
          ) : (
            <>
        ...
        <div style={{ transform: `scale(${sizePercent / 100})`, transformOrigin: 'top center' }}>
          {displayedResponses.length === 0 ? (
            <div className="p-12 rounded-2xl bg-white/40 dark:bg-slate-800/40 backdrop-blur-sm border border-white/60 dark:border-slate-700/50 text-center">
              <i
                aria-hidden="true"
                className="fi fi-sr-inbox text-4xl text-zinc-300 dark:text-zinc-600 block mb-3"
              />
              <p className="text-zinc-500 dark:text-zinc-400">
                No responses yet. Share the session code with students!
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {activeStepTab === -1 && hasSteps
                ? steps.map((step, sidx) => {
                    const stepRes = responses.filter(
                      (r: { stepIndex?: number }) => r.stepIndex === sidx,
                    );
...
function QuizResults({
  ...
  responses: {
    ...
    studentName?: string;
    mascot?: string;
    content: { score?: number; total?: number; answers?: Record<string, number> };
  }[];
  session: {
    config: { questions?: { question?: string; options?: string[]; correctAnswer?: number }[] };
  };
  onDelete: (id: string) => void;
}) {
          ...
          {perQuestionStats.map((stat) => {
            const pct =
              stat.totalAnswered > 0
                ...
                : 0;
            const optLabels = stat.options.map(
              (o: string, i: number) => `${String.fromCharCode(65 + i)}. ${o}`,
            );
            return (
              <div
                key={stat.qi}
                className="p-5 rounded-xl bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border border-white/60 dark:border-slate-700/50 shadow-sm"
              >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="space-y-1">
                      {stat.options.map((opt: string, oi: number) => {
                        ...
                        const optPct =
                          stat.totalAnswered > 0
                            ? Math.round((count / stat.totalAnswered) * 100)
                            : 0;
                        return (
                          <div
                            key={oi}
                          ...
                          >
                            <span className={`font-medium ${isCorrect ? 'font-bold' : ''}`}>
                              {optLabels[oi]}
                            </span>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between text-sm">
                    <span className="text-zinc-500 dark:text-zinc-400 font-medium">
                      {t('correct', {
                        n: String(stat.correctCount),
                        total: String(stat.totalAnswered),
                      })}
                    </span>
...
function ExitTicketResults({
  ...
  responses: {
    ...
    studentName?: string;
    mascot?: string;
    content: { learned?: string; question?: string; wantToKnow?: string };
  }[];
  onDelete: (id: string) => void;
}) {
  return (
          ...
          <div className="space-y-3">
            {responses
              ...
              .map((r) => (
                <div
                  key={r._id}
                  className="pb-3 border-b last:border-0 border-zinc-100 dark:border-slate-700/50"
                >

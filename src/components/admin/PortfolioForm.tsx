...
const RichTextEditor = dynamic(() => import('./RichTextEditor'), {
  ssr: false,
  loading: () => <div className="h-48 rounded-xl bg-zinc-100 dark:bg-slate-800 animate-pulse" />,
});
...
export default function PortfolioForm({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const totalBytes =
        (coverFile ? coverFile.size : 0) + newPhotoFiles.reduce((acc, f) => acc + f.size, 0);
      ...
      if (totalBytes > 0) {
        ...
        if (coverFile) {
          ...
          setProgress(30);
          finalCoverUrl = await uploadFileWithProgress(
            coverFile,
            'portfolio',
            () => {},
            signal,
        ...
        if (newPhotoFiles.length > 0) {
          ...
          const urls = await uploadFilesInBatches(
            ...
            (completed, total) => {
              ...
              const uploadedBytes = newPhotoFiles
                .slice(0, completed)
      ...
      <div className="space-y-6">
        <div className="p-6 rounded-2xl bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border border-white/60 dark:border-slate-700/50 shadow-sm space-y-4">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs text-zinc-500">Click to upload. Max 5MB. JPG, PNG, WEBP.</p>
          </div>
        ...
        >
          {isSubmitting ? 'กำลังบันทึก...' : isEdit ? 'อัปเดตข้อมูล' : 'สร้างผลงาน'}
        </button>
      </div>

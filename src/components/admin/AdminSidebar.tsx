  ...
  return (
    <>
      ...
      >
        {/* Brand */}
        <div
          className={`h-16 flex items-center justify-between border-b border-blue-100 dark:border-slate-800 ${collapsed ? 'px-0' : 'px-6'}`}
        >
          {!collapsed && (
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.href}
                  href={item.href}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-[background-color,color,transform] active:scale-95 active:translate-x-0.5 active:duration-press ${
                    isActive
                ...
                >
                  <i
                    className={`fi ${item.icon} ${isActive ? 'text-blue-600 dark:text-blue-400' : 'text-zinc-400'}`}
                  />
                  {item.label}
                </Link>
              );

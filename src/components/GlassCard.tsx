  ...
  return (
    <Tag
      className={`rounded-2xl bg-white/60 dark:bg-slate-800/60 backdrop-blur-sm border border-white/60 dark:border-slate-700/50 shadow-sm dark:shadow-black/20 transition-colors duration-500 ${
        ...
        hover
          ? `hover:bg-white/80 dark:hover:bg-slate-700/80 hover:-translate-y-0.5 hover:shadow-md dark:hover:shadow-black/40 transition-[background-color,border-color,transform,box-shadow] duration-200 ease-spring${Tag !== 'div' ? ' active:scale-[0.98] active:shadow-lg active:duration-press' : ''}`
          : ''
      } ${className}`}
    >
      {children}

/**
 * LawLib — reader-core shared helpers (Wave 2, lane B2).
 *
 * Owned by the reader-core lane. NOT part of the frozen parser/validate
 * surface (`src/lib/lawlib/**` untouched) — everything here derives from the
 * frozen LawDoc model and is consumed by client components only
 * (ArticleView / LawTooltip / TocSidebar / LawlibReaderClient).
 */

import type { Article, Chapter, LawDoc } from '@/types/lawlib';
import { SHORT_TERM_ALLOWLIST } from '@/lib/lawlib/terms';
...
export function loadCrossLaw(lawCode: string): Promise<LawDoc | null> {
  ...
  const promise = import('@/data/lawlib/registry')
    .then((mod) => {
